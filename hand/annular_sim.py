...
def actuation_annular(distance, q, current_pose, links_lenghts, integral, timestep):
    q1 = q[0]
    q2 = q[1]
    q3 = q[2]    
    dr_dot = 0

    offset_annular_y = links_lenghts[0]
    offset_annular_z = links_lenghts[1]
    L1y_annular = links_lenghts[2]
    L1z_annular = links_lenghts[3]
    L2y_annular = links_lenghts[4]
    L2z_annular = links_lenghts[5]
    L3y_annular = links_lenghts[6]
    L3z_annular = links_lenghts[7]
    

    #print(np.linalg.norm(current_pose - np.array([0, 0, 1])))

    py_j2 = offset_annular_y - L1y_annular*cos(q1) - L1z_annular*sin(q1)
    pz_j2 = offset_annular_z - L1y_annular*sin(q1) + L1z_annular*cos(q1)
        
    py_j3 = py_j2 - L2y_annular*cos(q1+q2) - L2z_annular*sin(q1+q2) 
    pz_j3 = pz_j2 - L2y_annular*sin(q1+q2) + L2z_annular*cos(q1+q2)

    py_tip = py_j3 - L3y_annular*cos(q1+q2+q3) - L3z_annular*sin(q1+q2+q3) + 0.2
    pz_tip = pz_j3 - L3y_annular*sin(q1+q2+q3) + L3z_annular*cos(q1+q2+q3) - 1.1

    p = np.array([[py_tip], [pz_tip]])    
    
    # Task Jacobian: p_dot = Jp(q)q_dot
    Jp = np.array([[L1y_annular*sin(q1) - L1z_annular*cos(q1) + L2y_annular*sin(q1+q2) - L2z_annular*cos(q1+q2) + L3y_annular*sin(q1+q2+q3) - L3z_annular*cos(q1+q2+q3), 
